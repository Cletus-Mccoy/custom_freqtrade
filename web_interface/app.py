...
# Endpoint to clear the cloudflared token: removes .env and resets token_set
@app.route('/api/cloudflared/clear-token', methods=['POST'])
def clear_cloudflared_token():
    env_path = BASE_PATH / 'web_interface' / 'cloudflared' / '.env'
    config_path = BASE_PATH / 'web_interface' / 'config' / 'user_config.json'
    # Remove .env file if it exists
    if env_path.exists():
        env_path.unlink()
    # Reset token_set in config
    if config_path.exists():
        with open(config_path, 'r') as f:
            config = json.load(f)
        cloudflare = config.get('global_settings', {}).get('cloudflare', {})
        cloudflare['token_set'] = False
        config['global_settings']['cloudflare'] = cloudflare
        with open(config_path, 'w') as f:
            json.dump(config, f, indent=2)
    return jsonify({'success': True})

# Utility: Safely update or add key-value pairs in a .env file
def update_env_file(env_path, updates):
    """
    Safely update or add key-value pairs in a .env file.
    ...
    with tempfile.NamedTemporaryFile('w', delete=False, dir=str(env_path.parent)) as tf:
        written = set()
        for line in lines:
            if '=' in line and not line.strip().startswith('#'):
                k = line.split('=', 1)[0].strip()
                if k in updates:
                    tf.write(f"{k}={updates[k]}\n")
                    written.add(k)
                else:
                    tf.write(line)
            else:
                tf.write(line)
        # Add any new keys not already written
        for k, v in updates.items():
            if k not in written:
                tf.write(f"{k}={v}\n")
    shutil.move(tf.name, env_path)

@app.route('/api/cloudflared/setup-and-launch', methods=['POST'])
def cloudflared_setup_and_launch():
    """
    Sets up the tunnel config, saves the .env, and launches the cloudflared container with config, cert, and env.
    """
    config_path = BASE_PATH / 'web_interface' / 'config' / 'user_config.json'
    cloudflared_dir = BASE_PATH / 'web_interface' / 'cloudflared'
    env_path = cloudflared_dir / '.env'
    config_yml_path = cloudflared_dir / 'config.yml'
    docker_template_path = cloudflared_dir / 'docker_templates' / 'cloudflared-compose.yml'
    data = request.get_json(force=True, silent=True) or {}
    ...
    if subdomain:
        env_vars['TUNNEL_SUBDOMAIN'] = subdomain
    if env_vars:
        lines = []
    ...
    try:
        subprocess.run(['docker', 'rm', '-f', 'cloudflared-dev'], check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    except Exception:
        pass  # Ignore errors if container does not exist
    # Launch the container using docker compose (with config, cert, env)
    compose_file = str(docker_template_path)
    try:
        subprocess.run([
            'docker-compose',
            '-f', compose_file,
        ...
        return jsonify({'success': False, 'error': f'Failed to launch container: {e}'})
